...
length_fuselage = 0 #7.5*radius_fuselage*2
wall_height = 10
radius_fuselage = 0 #length_fuselage/thickness_ratio_ellipse
offset_fuselage_vertical = 0
offset_fuselage_wing = 0
...
# chordiwse panels
m = 10*4
m_radial_elem_fuselage =m
# spanwise elements
n_elem_multiplier = 20
...
def generate_fem():
    ...
    plt.plot(y[wn:wn + n_node_main],z[wn:wn +  n_node_main], 'x', label = "Right wing")
    ...
    y[wn:wn + n_node_main] = np.linspace(0, -span_main, n_node_main)
    y[wn:wn + n_node_main] -= radius_fuselage
    plt.plot(y[wn:wn + n_node_main],z[wn:wn +  n_node_main], 'x', label = "Left wing")
    ...
    z[wn:wn + n_node_fuselage] = np.linspace(0.0, wall_height, n_node_fuselage-2)
    ...
    plt.plot(y[wn:wn + n_node_fuselage],z[wn:wn + n_node_fuselage], 'x',label = "Fuselage")
    plt.legend()
    plt.xlabel("y")
    plt.ylabel("z")
    plt.grid()
    plt.savefig("./wall_wing_nodes.png")
    plt.show()
    ...
    conn[we:we+n_elem_fuselage,:] -= 2
    conn[we, 0] = 0
    # conn[we,0] = np.array([idx_fuselage_wing_junctionconn[0,0],conn[n_elem_main,0],idx_fuselage_wing_junction])
    np.savetxt("connectivities.csv", conn)
    for inode in range(n_node_elem):
...
def generate_aero_file():
    ...
    i_surf = 0
    junction_boundary_condition_aero[wn] = 0 #1 # BC at fuselage junction that Zirkulation = Zirkulation
    ...
    i_surf = 1
    junction_boundary_condition_aero[wn] = 0#1 # BC at fuselage junction
    airfoil_distribution[we:we + n_elem_main, :] = 0
    surface_distribution[we:we + n_elem_main] = i_surf
    surface_m[i_surf] = m
    aero_node[wn:wn + n_node_main] = True
...
def generate_nonlifting_body_file():
    ...
    radius[wn:wn + n_node_fuselage] = 0
